...
import { PageDesignCore as Provider, DragItem, DropContainer, DropItem, DragLayer } from "@/src";
	...
	return (
		<DropItem item={item}>
			{({ item, connectDragAndDrop, isDragging }) => {
				return (
					<div ref={connectDragAndDrop} className="preview-field-wrapper">
						<div
						...
						{item.xtype === "GroupField" ? <GroupContainer item={item} /> : null}
					</div>
				);
			}}
		</DropItem>
...
export default class FormDesigner extends React.Component {
		...
		return (
			<div className="form-designer">
				...
				<div className="designer-container">
					<DropContainer id={null}>{this.renderFieldList}</DropContainer>
				</div>
			</div>
		);
	}
	...
	render() {
		...
		return (
			<Provider value={items} onChange={this.handleChange} ref={(p) => (window.p = p)}>
				{this.renderLayout()}
			</Provider>
		);
	}
