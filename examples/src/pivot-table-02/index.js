...
function DropFieldContainer({ id: pid }) {
	const _canDrop = React.useCallback(
		({ item, model }) => {
			if (item.id === "∑" && (pid === "value" || pid === "filter")) return false;
			...
			const items =
				pid === "filter"
					...
					: [...model.getChildren("column"), ...model.getChildren("row")];
			...
			{({ items, connectDropTarget, canDrop, isStrictlyOver, model }) => {
				return (
					...
					>
						{items.map((item) => {
							return (
								<DropItem canDrop={_canDrop} item={item} key={item.id}>
									{({
										isDragging,
										connectDropTarget,
										connectDragSource,
										...
										return (
											<div
												...
												className={
													"field-drop-item hover-dir-" + hoverDirection
												}
												ref={connectDropTarget}
											>
												<div
