...
class DropItem<T extends Item> extends React.Component<DropItemProps<T>> {
	...
	getHoverDirection(
		...
		const dragOffset = monitor.getClientOffset() || { x: 0, y: 0 };
	...
	getDropOptions() {
		...
		return {
			...
			canDrop: (dragResult, monitor: DropTargetMonitor) => {
				...
				let ret = model.isTmpItem(item) ? false : !model.isSameItem(item, dragItem);
			...
			hover: (dragResult: Required<DragObject<T>>, monitor: DropTargetMonitor) => {
				...
				const currentDirection = this.getHoverDirection(monitor, targetDOM, axis);
	...
	render() {
		...
		const [collectedDropProps, connectDropTarget] = useDrop(this.getDropOptions());

		const [collectedDragProps, connectDragSource, connectDragPreview] = useDrag(
			this.getDragOptions()
		);
		...
		props.hoverDirection =
			isStrictlyOver && !isDragging && canDrop ? this._lastHoverDirection : DRAG_DIR_NONE;
