...
import { ModelContext, ModelContextValue, DropContainerContextValue } from "./ModelContext";
	...
	commitDragStateItem(dragState: DragStateType) {
		...
		if (isNew) {
			...
			if (hoverItem) {
				...
				const isExist = find(childs, (item) => this.isSameItem(item, dragItem));
	...
	render() {
		...
		return (
			<ModelContext.Provider value={this.getModel()}>
				<DropZone>{isFunction(children) ? children(this) : children}</DropZone>
